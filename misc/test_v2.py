...
def main():
    ...
    keywords = ["asc", "desc", "rand"]
    checkpoint_files = [f for f in os.listdir(opts.checkpoint_dir) if f.endswith('.pth') and any(keyword in f for keyword in keywords)]
    ...
    csv_file = os.path.join(opts.checkpoint_dir, 'validation_results.csv')
    with open(csv_file, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Checkpoint", "Metrics"])
        for result in results:
            writer.writerow([result["checkpoint"], metrics.to_str(result["metrics"])])
