...
def main():
    ...
    for bucket_idx in range(opts.bucketidx ,opts.buckets_num):
        ...
        if len(train_label_paths) > 0:
            ...
            print(f"[INFO] Predicted labels for {len(train_label_paths)} training images")

        if len(val_label_paths) > 0:
            predict_labels(model, val_label_paths, val_label_dir)
            print(f"[INFO] Predicted labels for {len(val_label_paths)} validation images")
            ...
            for (images, labels) in train_loader:
                ...
                if (cur_itrs) % opts.val_interval == 0:
                    save_ckpt('checkpoints/latest_bucket_%s_%s_%s_os%d.pth' %
                            (bucket_idx ,opts.model, "kitti", opts.output_stride))
                    print("validation...")
                    model.eval()
                    val_score, ret_samples = validate(
