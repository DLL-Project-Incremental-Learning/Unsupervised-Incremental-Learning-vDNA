...
def finetuner(opts, model, teacher_model, checkpoint, bucket_idx, train_image_paths, train_label_dir, model_name , bucket_order = "rand", datetime = None):
    ...
    #    {'params': [p for n, p in model.backbone.layer4.named_parameters() if 'bias' in n and p.requires_grad]},
    #    {'params': [p for n, p in model.classifier.named_parameters() if 'bias' in n and p.requires_grad]}
    #    #{'params': model.classifier.parameters()}
    #    ]
    
    # If you have a classifier separate from the segmentation head, include it as well
    #if hasattr(model, 'classifier'):
    #    params_to_optimize.append({'params': model.classifier.parameters()})
